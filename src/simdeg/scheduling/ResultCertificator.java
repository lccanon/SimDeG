...
public abstract class ResultCertificator {
	 ...
	 * characteristics.
	 */
	abstract <J extends Job, R extends Result> R selectBestResult(
	 ...
	 */
	abstract <J extends Job, R extends Result> R selectLessWorseResult(
			List<Worker> workers, List<R> results);

	/**
	 * Utility function to sort the jobs by their respective result.
	 */
	protected static <J extends Job, R extends Result> Map<R, Set<Worker>> getJobsByResult(
			List<Worker> workers, List<R> results) {
